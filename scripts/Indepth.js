window.onload = function(){
    ...
    httpRequest.open('GET','http://localhost/Info2180-project2/database/Main.php?context=details&id='+params[0]);
    httpRequest.send();
    ...
    inProgress.addEventListener('click',function(){
        ...
        httpRequest.open('GET','http://localhost/Info2180-project2/database/Main.php?context=setStatus&status=inProgress&id=' + params[0]);
        httpRequest.send();
        location.reload();
        if (httpRequest.readyState == 4 && httpRequest.status == 200){
    ...
    markClosed.addEventListener('click',function(){
        httpRequest = new XMLHttpRequest;
        httpRequest.open('GET','http://localhost/Info2180-project2/database/Main.php?context=setStatus&status=closed&id=' + params[0]);
        httpRequest.send();
        location.reload();
        if (httpRequest.readyState == 4 && httpRequest.status == 200){
